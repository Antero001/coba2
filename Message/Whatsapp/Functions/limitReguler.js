...
exports.isLimit = function(sender, isPremium, isOwner, limitCount, _db){
    ...
    if (found === false) {
        ...
        _db.push(obj)
        fs.writeFileSync('./Database/Reguler_bot/limit.json', JSON.stringify(_db))
        return false
    }
}
...
exports.addBalanceReguler = function(sender, duit, _db){
    let position = false
    Object.keys(_db).forEach((i) => {
        if (_db[i].id === sender) {
            position = i
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./Database/Reguler_bot/balance.json', JSON.stringify(_db))
    } else {
        const bulin = ({
            id: sender,
            balance: duit
...
exports.isGame = function(sender, isOwner, gcount, _db){
    ...
    if (found === false){
        ...
        _db.push(obj);
        fs.writeFileSync('./Database/Reguler_bot/glimit.json',JSON.stringify(_db));
        return false;
    }
}
...
exports.createHit = function(sender, _db){
    ...
    _db.push(anohoh);
    fs.writeFileSync('./Database/Reguler_bot/userhit.json',JSON.stringify(_db));''
